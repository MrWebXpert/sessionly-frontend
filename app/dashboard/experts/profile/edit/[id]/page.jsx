...
const Page = () => {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                const { username, email, phone } = response.data.verifyStaff;
                console.log("userdata in edit page", response.data.verifyStaff)
    ...
    const updateAdmin = async (e) => {
        e.preventDefault()
        try {
            const response = await axios.patch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v2/staff/update/${id}`, formaData)
            console.log(response)
            toast.success(response.data.message)
            router.push('/dashboard/experts/profile')
        } catch (error) {
